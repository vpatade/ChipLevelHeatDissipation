...
void* adjustFanWRTTemp(void* args) {
	fprintf(stdout,"\nStarted Temperature Monitoring Thread\n");
	thread_param_t* arg = (thread_param_t*)args;
    package_config_t* pckgConfig = arg->pckgConfig;
    RC_model_t* model = arg->model;
    double* temp = arg->temp;
    assert(temp);
    assert(model);
    flp_t* flp = NULL;
    int i,rpm;
	...
	if (model->type == BLOCK_MODEL)
		...
		flp = model->grid->layers[0].flp;
    while(flp != NULL) {
    	//Read the temperature
    	for (i=0; i < flp->n_units; i++) {
    			if(strcmp(flp->units[i].name,"IntReg_0") ==0 || strcmp(flp->units[i].name,"IntReg_1") ==0) {
    				temperature = temp[i];
    				break;
    			}
    	}
    	...
    	if(temperature > 70.00) {
    		...
    		rpm = -0.0031*pow(temperature,6) + 0.1576*pow(temperature,5)
    		       - 2.929*pow(temperature,4) + 26.352*pow(temperature,3)
    		       - 127.66*pow(temperature,2) + 323.1*pow(temperature,1)
    		        + 3010.1*pow(temperature,0);
    		pckgConfig->rpm = rpm;
    	}
    	sleep(200);
    }
...
void* adjustTempWRTFan(void* args) {
	fprintf(stdout,"\nStarted Fan Monitoring Thread\n");
	thread_param_t* arg = (thread_param_t*)args;
    package_config_t* pckgConfig = arg->pckgConfig;
    RC_model_t* model = arg->model;
    double* temp = arg->temp;
    ...
    while(flp != NULL) {
    	...
    	temperature = 2*pow(10,-9)*pow(rpm,5) - 3*pow(10,-5)*pow(rpm,4)
    			      + 0.185*pow(rpm,3) - 617.07*pow(rpm,2)
    	              + 1*pow(10,6)*pow(rpm,1) - 7*pow(10,8);
    	//Set the temperature
    	for (i=0; i < flp->n_units; i++) {
    		if(strcmp(flp->units[i].name,"IntReg_0") ==0 || strcmp(flp->units[i].name,"IntReg_1") ==0) {
    				temp[i] = temperature;
...
int main(int argc, char **argv)
{
	...
	while ((num=read_vals(pin, vals)) != 0) {
		...
		if (do_transient) {
		    ...
		    pthread_t fanThread;
		    package_config_t p= default_package_config();
		    thread_param_t thread_param;
		    thread_param.temp = vals;
		    thread_param.model = model;
		    thread_param.pckgConfig = &p;

            pthread_create(&tempThread,NULL,(void*)&adjustTempWRTFan,(void*)&thread_param);
            pthread_create(&fanThread,NULL,(void*)&adjustFanWRTTemp,(void*)&thread_param);
